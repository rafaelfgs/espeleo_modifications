...
class odometry:
    ...
    def motor6_callback(self, message):
        ...
        self.current_time = message.header.stamp.to_sec()
    ...
    def odometry_calculation(self):
        ...
        self.odom_broadcaster.sendTransform(
            ...
            rospy.get_param("~child_frame_id", "odom_pose"), # base_link
            rospy.get_param("~frame_id", "odom_init"), # odom
        )

        # next, we'll publish the odometry message over ROS
        odom = Odometry()
        odom.header.stamp = rospy.Time.from_sec(self.current_time) # rospy.Time.now()
        odom.header.frame_id = rospy.get_param("~frame_id", "odom_init") # odom

        # set the position
        odom.pose.pose = Pose(Point(self.x, self.y, 0.), Quaternion(*odom_quat))

        # set the velocity
        odom.child_frame_id = rospy.get_param("~child_frame_id", "odom_pose") # base_link
